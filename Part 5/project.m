...
for i=1:Ns+1
    ...
    heading_noise   = normrnd(0,deg2rad(0.5));
    rate_noise      = normrnd(0,deg2rad(0.1));
    
    eta_noise = eta + [0;0;heading_noise];
    nu_noise = nu + [0;0;rate_noise];
    
    K_kalman = P_pred*C_kd' * inv(C_kd*P_pred*C_kd.'+ R_d);
...
%         e_int = e_int + h/Ki * (delta_c - u_unsat);
    ...
    simdata(i,:) = [t n_c delta_c n delta eta' nu' u_d psi_d...
        r_d chi_d eta_noise(3) nu_noise(3) x_hat'];       
...
psi_msm = (180/pi) * simdata(:,16);
r_msm   = (180/pi) * simdata(:,17);
psi_hat = (180/pi) * simdata(:,18);
r_hat   = (180/pi) * simdata(:,19);
...
figure(5)
subplot(211)
plot(t,psi_msm, t, psi,'linewidth',2);
legend('Measured $\psi$', 'True $\psi$','Interpreter','latex');
title('Heading vs noisy heading')
subplot(212)
plot(t,r_msm, t, r,'linewidth',2);
legend('Measured $r$', 'True $r$', 'Interpreter','latex')
